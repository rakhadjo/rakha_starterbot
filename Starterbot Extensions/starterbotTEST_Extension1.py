...
def playGame(guessing):
	...
	try:
		...
		if len(guess) > 0:
			slack_client.api_call("chat.postMessage", channel=channel, text="U cant do this fam pls pick one number only ", as_user=True)
		else:
...
def getToKnowAns2(jawaban2):
	global zum
	#slack_client.api_call("chat.postMessage", channel=channel, text="Where u from", as_user=True)
	global tempat
	#global surveyMode
	tempat = jawaban2
